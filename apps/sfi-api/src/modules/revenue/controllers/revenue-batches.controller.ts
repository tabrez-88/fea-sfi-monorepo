...
export class RevenueBatchesController {
  ...
  @ApiQuery({ name: 'search', required: false, type: String, description: 'Case-insensitive search on batchNumber OR source (Screen 3.1 search bar)' })
  @ApiQuery({ name: 'periodFrom', required: false, type: String, description: 'ISO date — return only batches whose periodStart is on/after this' })
  @ApiQuery({ name: 'periodTo', required: false, type: String, description: 'ISO date — return only batches whose periodEnd is on/before this' })
